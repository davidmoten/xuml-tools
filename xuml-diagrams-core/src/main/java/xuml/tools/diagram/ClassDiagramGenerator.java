...
public class ClassDiagramGenerator {
    ...
    private Optional<Subsystem> getSubsystem(Domains domains, String domainName,
    ...
    private void generateGeneralization(StringBuilder s, Generalization g) {
        for (Named sp : g.getSpecializedClass())
            s.append("<div class=\"generalization\" id=\"" + replaceSpaces(sp.getName()) + "-"
    ...
    private void generateAssociation(StringBuilder s, BinaryAssociation r, Subsystem ss) {
        ...
        s.append(
                "<div class=\"relationship\" id=\"" + getRelationshipName(r.getRnum())
                        + "\" className1=\"" + r.getActivePerspective().getViewedClass()
                                .replaceAll(" ", "_")
                        + "\" className2=\""
                        + r.getPassivePerspective().getViewedClass().replaceAll(" ", "_")
                        + "\" verbClause1=\"" + r.getActivePerspective().getPhrase()
                        + "\" verbClause2=\"" + r.getPassivePerspective().getPhrase()
                        + "\" multiplicity1=\""
                        + getMultiplicityAbbreviation(r.getActivePerspective())
                        + "\" multiplicity2=\""
                        + getMultiplicityAbbreviation(r.getPassivePerspective()) + "\"></div>\n");
    }

    private void generateAssociation(StringBuilder s, UnaryAssociation r, Subsystem subsystem) {
    ...
    private void generateClass(StringBuilder s, xuml.tools.miuml.metamodel.jaxb.Class cls) {
        ...
        s.append("<div id=\"" + cls.getName().replaceAll(" ", "_") + "\" class=\"cls draggable");
        ...
        s.append("\"");
        if (assocationClassRelationship != null)
            s.append(" relationshipName=\"" + getRelationshipName(assocationClassRelationship)
                    + "\" ");
        s.append(">\n");
        ...
        for (JAXBElement<? extends Attribute> attr : cls.getAttribute()) {
            ...
            if (attr.getValue() instanceof IndependentAttribute) {
                ...
                s.append("    <div class=\"attribute\">" + a.getName() + ": " + a.getType() + " ");
                // if (!a.isMandatory())
                // items.add("O");
                ...
                s.append("<div class=\"attribute\">" + "/ " + d.getName() + ": " + d.getType());
            }
        ...
        // s.append("</div>");
        // }
        if (cls.getLifecycle() != null && !cls.getLifecycle().getEvent().isEmpty()) {
            s.append("<div class=\"events\">");
            for (JAXBElement<? extends Event> event : cls.getLifecycle().getEvent()) {
    ...
    private BigInteger getAssociationClassRelationship(Class cls) {
        for (JAXBElement<? extends Attribute> element : cls.getAttribute())
            if (element.getValue() instanceof ReferentialAttribute) {
                ReferentialAttribute ref = (ReferentialAttribute) element.getValue();
