...
public class StateDiagramViewer {
	...
	private void open(ModeledDomain domain) {
		if (classMenu == null)
			throw new RuntimeException(
					"must call start() before calling open(domain)");
		classMenu.removeAll();
	...
	public void show(Class c) {
		...
		if (c.getLifecycle() != null) {
			c.getLifecycle().getState().stream()
					.forEach(state -> g.addVertex(state.getName()));
			c.getLifecycle()
					.getTransition()
					.stream()
					.forEach(
							transition -> {
								...
								String eventName = c
										...
										.findAny().get();
								g.addEdge(new Edge(eventName), fromState,
										toState);
							});
		}
				...
				.invokeLater(() -> {
					...
					saveAsImageAction.addActionListener(e -> {
						...
						if (returnVal == JFileChooser.APPROVE_OPTION) {
							...
							try {
								vvContainer.setPreferredSize(vvContainer
										.getSize());
	...
	private static VisualizationViewer<String, Edge> createVisualizationViewer(
		...
		FRLayout<String, Edge> layout = new FRLayout<String, Edge>(graph,
				new Dimension(800, 600));
		while (!layout.done())
			layout.step();

		VisualizationViewer<String, Edge> vv = new VisualizationViewer<String, Edge>(
				layout, new Dimension(800, 600));
		vv.getRenderContext().setVertexLabelTransformer(s -> s);
		vv.getRenderContext().setVertexFillPaintTransformer(
				vertex -> vertex.equals("Created") ? Color.decode("#B5D9E6")
						: Color.decode("#FFF1BC"));
		vv.getRenderContext().setVertexDrawPaintTransformer(
				vertex -> Color.black);
		vv.getRenderContext().setVertexShapeTransformer(
				createVertexShapeTransformer(layout));
		vv.getRenderer().getVertexLabelRenderer()
				.setPosition(Renderer.VertexLabel.Position.CNTR);
		vv.getRenderContext().setEdgeLabelTransformer(edge -> edge.name);
		vv.getRenderContext().setEdgeShapeTransformer(
				new EdgeShape.QuadCurve<String, Edge>());
	...
	private static Transformer<String, Shape> createVertexShapeTransformer(
		...
		return vertex -> {
			...
			int ascent = 12;
			int descent = 5;
			Shape shape = new Rectangle(-w / 2 - margin, -ascent - margin, w
					+ 2 * margin, (ascent + descent) + 2 * margin);
			return shape;
		};
	}
	...
	public void start(String resource, String domainName) {
		start(StateDiagramViewer.class.getResourceAsStream(resource),
				domainName);
	}
