...
public class SelectBuilderTest {
    ...
    private final StringExpressionField<Ent> g = new StringExpressionField<Ent>(new Field("field"));
    ...
    public void testClauseGenerationPlus() {
        assertEquals("((e.field + e.field) <= :_p1)", builder(f.plus(f).lte(3)).getClause());
    }

    @Test
    public void testClauseGenerationMinus() {
        assertEquals("((e.field - e.field) <= :_p1)", builder(f.minus(f).lte(3)).getClause());
    }

    @Test
    public void testClauseGenerationTimes() {
        assertEquals("((e.field * e.field) <= :_p1)", builder(f.times(f).lte(3)).getClause());
    }

    @Test
    public void testClauseGenerationDivide() {
        assertEquals("((e.field / e.field) <= :_p1)", builder(f.divide(f).lte(3)).getClause());
    }
